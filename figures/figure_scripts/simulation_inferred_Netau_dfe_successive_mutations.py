...
path_folder_all = '../../simulations/simulation_data/dfe_successive_mutations/'

shapes = ['gaussian', 'rectangular', 'constant']
for shape in shapes:
    path_folder = path_folder_all + shape
    ...
    ax[0].set_title('DFE of deleterious mutations from Bloom and Neher')
    ...
    ax[3].plot(true_c['Epiweek'], true_c['c_true'])
    ax[3].set_ylim([0, 13])
    ...
    plt.tight_layout()
    plt.savefig('../figure_outputs/simulation_inferred_Netau_dfe_successive_mutations_' + shape + '.png', dpi = 300)
    plt.savefig('../figure_outputs/simulation_inferred_Netau_dfe_successive_mutations_' + shape + '.pdf')
