...
path_folder_all = '../../simulations/simulation_data/neutral_nb_offspring_dist/'
...
for shape in shapes:
    ...
    ax[2].plot(summary['Epiweek'], summary['Netau_true']*65, label = 'Census population size')
    ax[2].set_yscale('log')
    ax[2].set_ylabel('Scaled effective\npopulation size,\n$\\tilde{N}_e(t)$')
    ax[2].set_ylim([10**2, 10**6])
    ax[2].set_yscale('log')
    ax[2].legend()
    ...
    plt.tight_layout()
    plt.savefig('../figure_outputs/simulation_inferred_Netau_' + shape + '_nb_offspring_dist.png', dpi = 300)
    plt.savefig('../figure_outputs/simulation_inferred_Netau_' + shape + '_nb_offspring_dist.pdf')
