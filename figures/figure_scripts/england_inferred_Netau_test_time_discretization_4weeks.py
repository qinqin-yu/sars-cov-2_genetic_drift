...
for i in range(len(variant_param_folders)):
    ...
    if os.path.exists(variant_param_folder + 'England/inference_results/summary_corrected.csv'): 
        ...
        delt2 = pd.read_csv(variant_param_folder + 'England/inference_results/summary_corrected_test_time_discretization_4weeks.csv', index_col = 0)
    else:
        delt1 = pd.read_csv(variant_param_folder + 'England/inference_results/summary.csv', index_col = 0)
        delt2 = pd.read_csv(variant_param_folder + 'England/inference_results/summary_test_time_discretization_4weeks.csv', index_col = 0)
    ...
    p = ax[row, col].plot(delt2['date'], delt2['Netau_HMM_median']*4, zorder = 10, color = 'tab:orange', marker = 'o', markeredgecolor = 'k', label = 'every 4 weeks')#, capsize = 5)
    ax[row, col].fill_between(delt2['date'], delt2['Netau_HMM_95%_ci_lower']*4, delt2['Netau_HMM_95%_ci_upper']*4, color=p[0].get_color(), alpha=0.2)
...
plt.tight_layout()
plt.savefig('../figure_outputs/england_inferred_Netau_test_time_discretization_4weeks.pdf')
plt.savefig('../figure_outputs/england_inferred_Netau_test_time_discretization_4weeks.png', dpi = 300)
