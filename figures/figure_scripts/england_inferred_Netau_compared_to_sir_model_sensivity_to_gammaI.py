...
summary_all = pd.read_csv('../../data/epidemiological_models/sensitivity_to_gammaI/epidemiological_models_Netau_inferred_Netau_by_variant_gammaI0.5_gammaE1.0.csv', parse_dates = ['date'], index_col = 0)
summary_all2 = pd.read_csv('../../data/epidemiological_models/sensitivity_to_gammaI/epidemiological_models_Netau_inferred_Netau_by_variant_gammaI2.33_gammaE7.0.csv', parse_dates = ['date'], index_col = 0)

labels = []

merged = pd.read_csv('../../data/epidemiological_models/sensitivity_to_gammaI/epidemiological_models_Netau_overall_gammaI0.5_gammaE1.0.csv', parse_dates = ['date'], index_col = 0)
merged2 = pd.read_csv('../../data/epidemiological_models/sensitivity_to_gammaI/epidemiological_models_Netau_overall_gammaI2.33_gammaE7.0.csv', parse_dates = ['date'], index_col = 0)

errcond = merged['Netau SIR error criteria met']
errcond2 = merged2['Netau SIR error criteria met']
...
p = ax[0].plot(merged2[errcond2]['date'], 2*merged2[errcond2]['Netau SIR'], label = '$N_e\\tau$ SIR', color = 'k', marker = 'v', markersize = 4)
ax[0].fill_between(merged2[errcond2]['date'], 2*merged2[errcond2]['Netau SIR lower'], 2*merged2[errcond2]['Netau SIR upper'], color=p[0].get_color(), alpha=0.2)
#labels.append('All lineages')
...
labels.append('$\\tilde{N}^{\mathrm{SIR}}(t), \gamma_I^{-1} = 14 \mathrm{d}$')#, \gamma_E^{-1} = 7\mathrm{d}$')
labels.append('$\\tilde{N}^{\mathrm{SIR}}(t), \gamma_I^{-1} = 3\mathrm{d}$')#, \gamma_E^{-1} = 1\mathrm{d}$')
#print
...
plt.tight_layout()
plt.savefig('../../figures/figure_outputs/england_inferred_Netau_compared_to_sir_model_sensitivity_to_gammaI.png', dpi = 300)
plt.savefig('../../figures/figure_outputs/england_inferred_Netau_compared_to_sir_model_sensitivity_to_gammaI.pdf')
plt.show()