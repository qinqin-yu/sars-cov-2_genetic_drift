...
#########
# RANDOMLY SUBSAMPLE 1/10 OF LINEAGES, AS CONTROL

variant_columns = ['B-1-177|2021-02-22|694.5',
                   'alpha|2021-06-20|61.5',
                   'delta|2022-01-25|49.5+58.5']
...
for variant_column in variant_columns:
    ...
    metadata_pillar2_subsample = metadata_pillar2.sample(n = int(len(metadata_pillar2)/10))
    
    output_folder = path_folder + variant + '/' + variant_column + '/is_pillar_2/England_random_subsample_one_tenth/'
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    
    save_counts_from_metadata(metadata_pillar2_subsample, variant_column, tree_column, output_folder)
