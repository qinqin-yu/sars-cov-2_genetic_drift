...
for i in range(len(Net_all)):
    Net = np.round(Net_all[i])
    Net_label = Net_all_labels[i]
    
    output_folder = path_folder + 'dfe_successive_mutations_control/' + Net_label + '/'
    ...
    wsf.run_simulation_dfe(path_folder, output_folder, counts_output_filename, total_counts_output_filename, fitness_filename, total_epiweeks, total_burnin_time, Net, ct, Nseq, mu, numlineages, dfe = [0])