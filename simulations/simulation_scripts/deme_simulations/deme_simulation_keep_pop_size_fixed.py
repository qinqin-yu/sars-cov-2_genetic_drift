...
output_path = '../../simulation_data/deme_simulations/counts_keep_pop_size_fixed/'
...
pop_size = 5.6*10**7 # England population size 
num_demes_all = pop_size/np.array(Ns)
...
i = 0
for N in Ns:
    num_demes = num_demes_all[i]
    print(N, num_demes, N*num_demes)
...
output_path = path_folder + 'counts_keep_pop_size_fixed_t0_42_tl_50/'
if not os.path.exists(output_path):
    os.makedirs(output_path)
filenames = glob.glob(path_folder + 'counts_keep_pop_size_fixed/demes_' + str(num_demes) + '_lineages_200_filleddemes0_*_demeN_*')
