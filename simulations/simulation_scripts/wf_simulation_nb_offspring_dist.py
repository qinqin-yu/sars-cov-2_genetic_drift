...
# Note that we multiply everything by a factor of 65 to take into account that the variance in offspring number will be 65
...
for i in range(len(Net_all)):
    ...
    output_folder = path_folder + 'neutral_nb_offspring_dist/' + Net_label + '/'
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
        
    ct_df = pd.DataFrame([ct], columns = t)
    Net_df = pd.DataFrame([Net/offspring_var], columns = t)
    ...
    wsf.run_simulation_nb_offspring_dist(path_folder, output_folder, counts_output_filename, total_counts_output_filename, fitness_filename, total_epiweeks, total_burnin_time, Net, ct, Nseq, mu, numlineages, offspring_var)
